...
static void processAudio(int16_t *out, int16_t *in) {
	...
	for (int n = 0; n < AUDIO_BUF_SIZE; n++) {
		...
		if(fillScratch<delay){
			...
			//out[n] = in[n];
			//out[n]= (float)volume*((newchorus1*depth) + (newchorus2*depth) + (newchorus3*depth) + (newchorus4*depth) + (newchorus5*depth) + (newchorus6*depth) + (newchorus7*depth) + (newchorus8*depth) + (newchorus9*depth) + (newchorus10*depth) + (newchorus11*depth) + (newchorus12*depth) + (newchorus13*depth) + (newchorus14*depth)+ ((float)in[n]*(1.0f-depth)))/1000.0f ; // chorus out[n]= (volume*depth)/100;

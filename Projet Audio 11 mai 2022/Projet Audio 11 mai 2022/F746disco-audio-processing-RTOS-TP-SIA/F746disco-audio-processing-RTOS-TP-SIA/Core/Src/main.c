 ...
 *
 *
 * Board peripherals organization:
 * - I2C1: Camera + ext Arduino compatible connector
 * - I2C3: Touchscreen & Audio CODEC
 * - SAI2: W8994 Audio Codec with Block A (master) = OUT and Block B (slave) = IN
 *
 * To do after regenerating code from Cube configurator:
 * - comment out both SCB_EnableCacheI/D() at lines 177-178 !!! (cache must be disabled before init'ing MPU, and enabled right after)
 *
 *
 * How to:
 * - change the audio sampling frequency: at line 864 edit hsai_BlockA2.Init.AudioFrequency = SAI_AUDIO_FREQUENCY_16K
 * - change the audio input: at line 904 set hsai_BlockB2.SlotInit.SlotActive=0x0005 for LineIn, 0x000A for MicIn.
 * This is enough as all these changes will be properly reflected in audio.c.
 *
 *
 ******************************************************************************
 */
...
#ifndef FB_IN_SDRAM
extern uint16_t frameBuf[LCD_SCREEN_WIDTH * LCD_SCREEN_HEIGHT];
#endif
  ...
  /* USER CODE BEGIN I2C1_Init 0 */
	...
	/*
     External, camera and Arduino connector  I2C configuration
		BSP indique :
		- hi2c1.Init.Timing = ((uint32_t)0x40912732);
		- pas de ligne hi2c1.Init.OwnAddress2Masks = I2C_OA2_NOMASK;
		TODO : vérifier
	 */
...
static void MX_I2C3_Init(void)
{
  ...
  /* USER CODE BEGIN I2C3_Init 0 */
	...
	/* audio, touchscreen
	 TODO dans le BSP on avait :

	 	 hi2c3.Init.Timing = ((uint32_t)0x40912732);

	 mais le touchscreen fonctionne aussi avec la valeur par defaut proposée par CubeMX
	 TODO vérifier avec l'audio
	 */
...
static void MX_SAI2_Init(void)
{
  ...
  /* USER CODE BEGIN SAI2_Init 0 */
	...
	// to change the audio input, set hsai_BlockB2.SlotInit.SlotActive=0x0005 for LineIn,
	// and hsai_BlockB2.SlotInit.SlotActive=0x0000000A for MicIn.

	// to change the sampling rate, edit hsai_BlockA2.Init.AudioFrequency (then block B will have the same rate as it is a slave to block A)
